...
import {
  Upload, Copy, Check, Shuffle, Plus, Minus,
...
const STRATEGY_ORDER: ExtractionStrategy[] = [
  "dominant", "vibrant", "muted", "light", "dark", "warm", "cool", "accent",
];
...
function kmeanspp(pixels: [number, number, number][], k: number): [number, number, number][] {
...
function runKmeans(pixels: [number, number, number][], k: number): Cluster[] {
  ...
  for (let iter = 0; iter < KMEANS_ITERATIONS; iter++) {
    ...
    centroids = centroids.map((old, c) => {
      ...
      return [sums[c][0] / counts[c], sums[c][1] / counts[c], sums[c][2] / counts[c]] as [number, number, number];
    });
  }
  ...
  return centroids
    .map((centroid, i) => ({ centroid, hex: oklabToHex(centroid[0], centroid[1], centroid[2]), count: clusterCounts[i] }))
    .filter((c) => c.count > 0);
}

function rankClusters(clusters: Cluster[], strategy: ExtractionStrategy, count: number): Cluster[] {
  let largeCentroids: [number, number, number][] | null = null;
  if (strategy === "accent") {
    const sorted = [...clusters].sort((a, b) => b.count - a.count);
    const largeThreshold = sorted[Math.floor(sorted.length / 3)]?.count ?? 0;
    largeCentroids = sorted.filter((cl) => cl.count >= largeThreshold).map((cl) => cl.centroid);
  }
  ...
  const scored = clusters.map((c) => {
    ...
    switch (strategy) {
      case "dominant": score = c.count; break;
      case "vibrant": score = chroma * (c.count > 0 ? 1 : 0); break;
      case "muted": score = 1 / (chroma + 0.001); break;
      case "light": score = L; break;
      case "dark": score = 1 - L; break;
      case "warm": {
...
export default function PaletteExtractorTool() {
  ...
  const extract = useCallback((canvas: HTMLCanvasElement) => {
    setExtracting(true);
  ...
  const handleSwatchClick = useCallback((i: number, e: React.MouseEvent) => {
    if (isTouchDevice) {
      e.stopPropagation();
    ...
    return (
      <div className="space-y-6">
        <div
          ...
          onClick={() => document.getElementById("palette-extractor-input")?.click()}
        >
          <input
  ...
  return (
    <div className="space-y-6">
                ...
                >
                  ...
                  >
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        copyValue(formatColour(cluster.hex, notation), `swatch-${i}`);
                      }}
                      className={cn(
                    ...
                    >
                      {copied === `swatch-${i}` ? (
                        ...
                        <><Copy className="size-4" />{cluster.hex.toUpperCase()}</>
                      )}
                    </button>
                  </div>
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-xl border bg-card p-3">
        <button
          onClick={() => document.getElementById("palette-extractor-input-change")?.click()}
        ...
        >
          {imageUrl && <img src={imageUrl} alt="Source" className="size-full object-cover" />}
        </button>
        <input
        ...
        <Select value={strategy} onValueChange={(v) => setStrategy(v as ExtractionStrategy)}>
          ...
          <SelectContent>
            {STRATEGY_ORDER.map((s) => (
              <SelectItem key={s} value={s}>{STRATEGIES[s].name}</SelectItem>
            ))}
          </SelectContent>
        </Select>

        <Button onClick={reExtract} disabled={extracting} variant="outline" size="sm" className="gap-2" title="Re-extract with new seeds (Space)">
          <Shuffle className="size-4" />
          Re-extract
        </Button>

        <div className="flex items-center gap-1 ml-auto">
          <Button variant="ghost" size="icon" className="size-8" onClick={() => setCount((c) => Math.max(MIN_COLOURS, c - 1))} disabled={count <= MIN_COLOURS} title="Fewer colours">
            <Minus className="size-3.5" />
          </Button>
          <span className="px-1 font-mono text-sm font-bold min-w-[2ch] text-center">{count}</span>
          <Button variant="ghost" size="icon" className="size-8" onClick={() => setCount((c) => Math.min(MAX_COLOURS, c + 1))} disabled={count >= MAX_COLOURS} title="More colours">
      ...
      <p className="text-sm text-muted-foreground">
        <span className="font-semibold text-foreground">{STRATEGIES[strategy].name}</span> — {STRATEGIES[strategy].description}
      </p>

      {palette.length > 0 && (
        <div className="flex flex-wrap items-center gap-2">
          <Button variant="outline" size="sm" onClick={copyAllColours} className="gap-2">
            {copied === "all" ? <Check className="size-3.5" /> : <Copy className="size-3.5" />}
            Copy All
          </Button>
          <Button variant="outline" size="sm" onClick={copyAsCss} className="gap-2">
            {copied === "css" ? <Check className="size-3.5" /> : <Copy className="size-3.5" />}
            CSS Variables
          </Button>
          {palette.length > 0 && (
            <Button asChild variant="outline" size="sm" className="gap-2 ml-auto">
              <Link
                to="/tools/palette-genny"
                search={{ colors: palette.map((c) => c.hex).join(",") }}
              >
      ...
      {palette.length > 0 && (
        <div className="grid gap-2">
          {palette.map((cluster, i) => {
            ...
            const pct = totalPixels > 0 ? ((cluster.count / totalPixels) * 100).toFixed(1) : "0";
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3">
                    <span className="font-mono font-bold tracking-wide">
                      {notation === "hex" ? cluster.hex.toUpperCase() : formatted}
                    </span>
                    <span className="text-sm text-muted-foreground capitalize truncate">{name}</span>
                  </div>
                  <div className="text-xs text-muted-foreground font-mono mt-0.5">{pct}% of image</div>
                </div>
                <div className="flex items-center gap-1 shrink-0">
                  <Button
                    ...
                    title="Copy colour"
                  >
                    {copied === `list-${i}` ? <Check className="size-4" /> : <Copy className="size-4" />}
                  </Button>
                </div>
              </div>
            );
      ...
      {palette.length > 0 && (
        <p className="text-xs text-muted-foreground text-center">
          Press <kbd className="px-1.5 py-0.5 rounded bg-muted font-mono">Space</kbd> to re-extract with new seeds
        </p>
      )}
    </div>
  );
