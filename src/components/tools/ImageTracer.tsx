...
const PRESETS: PresetConfig[] = [
  ...
  { id: "posterized2", label: "Saul's World", icon: Layers, description: "Medium posterisation" },
  { id: "posterized3", label: "Powders", icon: Layers, description: "Heavy posterisation" },
  { id: "curvy", label: "Bouba", icon: Spline, description: "Smooth organic curves" },
  { id: "sharp", label: "Kiki", icon: Triangle, description: "Precise angular lines" },
  { id: "detailed", label: "Intricate", icon: Scan, description: "High detail, many colours" },
  { id: "smoothed", label: "Papered", icon: Waves, description: "Gaussian blur pre-pass" },
  ...
  { id: "artistic4", label: "Sparkle", icon: Sparkles, description: "Abstract artistic" },
];
...
function StrokeWidthPicker({ value, onChange }: { value: number; onChange: (v: number) => void }) {
...
function ColourCountCard({ value, onChange }: { value: number; onChange: (v: number) => void }) {
  return (
  ...
  try {
    ...
    const workerCode = `
      ...
      self.onmessage = function(e) {
        var imgd = new ImageData(new Uint8ClampedArray(e.data.buffer), e.data.width, e.data.height);
...
export default function ImageTracerTool() {
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-1.5">
          <Button size="sm" onClick={handleDownload} disabled={!hasResult || tracing}>
