...
export default function Base64Tool() {
  ...
  const output = useMemo(() => {
    try { return mode === "enc" ? encoders.base64Encode(input) : encoders.base64Decode(input); }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <IOPanel label={mode === "enc" ? "Plain text" : "Base64"} value={input} onChange={setInput} />
        <IOPanel label={mode === "enc" ? "Base64" : "Plain text"} value={output} readOnly />
      </div>
    </div>
  );
}
