...
export default function BackgroundRemover() {
  ...
  useEffect(() => {
    return () => {
      const p = pipelineRef.current as { dispose?: () => void } | null;
      if (p?.dispose) {
        p.dispose();
  ...
  const removeBackground = async () => {
    ...
    try {
      ...
      const result = await (pipelineRef.current as (img: string) => Promise<unknown[]>)(sourceImage) as { mask: { toDataURL?: () => string; data?: Uint8Array; width?: number; height?: number } }[];
      ...
      if (result && result.length > 0 && result[0].mask) {
        ...
        if (typeof maskImage.toDataURL === "function") {
          ...
          tempCanvas.width = maskImage.width ?? 0;
          tempCanvas.height = maskImage.height ?? 0;
