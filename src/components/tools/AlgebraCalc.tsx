...
import { Copy, Check, Loader2, Sigma } from "lucide-react";
...
export default function AlgebraCalc() {
  ...
  const calculate = async () => {
    ...
    try {
      ...
      switch (operation) {
        case "simplify":
          output = nerdamer(expression).text("fractions");
          latex = nerdamer(expression).toTeX();
          break;

        case "expand":
          output = nerdamer("expand(" + expression + ")").text("fractions");
          latex = nerdamer("expand(" + expression + ")").toTeX();
          break;

        case "factor":
          output = nerdamer("factor(" + expression + ")").text("fractions");
        ...
        case "derivative":
          output = nerdamer("diff(" + expression + ", " + variable + ")").text("fractions");
          latex =
            "\\frac{d}{d" +
            variable +
            "}" +
        ...
        case "integral":
          output = nerdamer("integrate(" + expression + ", " + variable + ")").text("fractions") + " + C";
          latex =
            "\\int " +
            nerdamer(expression).toTeX() +
            " \\, d" +
      ...
      setResult({ input: expression, output, latex, operation });
    } catch (e) {
    ...
    <div className="space-y-6">
      ...
      {!nerdamerLoaded && (
        <div className="flex items-center justify-center gap-2 py-8 text-muted-foreground">
          ...
          <span className="text-sm">Loading Algebra engine...</span>
        </div>
      )}
      ...
      {nerdamerLoaded && (
        <>
          ...
          <div className="space-y-4">
            <div className="flex gap-3">
              <div className="flex-1 space-y-2">
                ...
                <Input
                  ...
                  placeholder={operation === "solve" ? "e.g., x^2 - 4 = 0" : "e.g., (x+1)^2 - x^2"}
            ...
            >
              {loading ? (
                <><Loader2 className="size-4 mr-2 animate-spin" />Calculating...</>
              ) : (
                "Calculate"
              )}
            </Button>
          ...
          {(result || error) && (
            <div
              className={"border rounded-lg p-6 " + (error ? "border-destructive bg-destructive/5" : "bg-card")}
            >
              {error ? (
                ...
                <div className="space-y-4">
                  ...
                  <div ref={resultRef} className="text-2xl font-mono overflow-x-auto py-4" />
