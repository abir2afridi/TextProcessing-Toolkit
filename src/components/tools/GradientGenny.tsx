    ...
    >
      ...
      >
        ...
        <div className="text-xs text-muted-foreground whitespace-nowrap">
          ({Math.round(parseFloat(x) * 100 / 100)}%, {Math.round(parseFloat(y) * 100 / 100)}%)
        </div>
      </div>
      ...
      <button
        ...
        onClick={() => inputRef.current?.click()}
        className={`
          w-6 h-6 rounded-full border-2 border-white cursor-grab active:cursor-grabbing
          transition-all duration-200 ease-out
          hover:scale-125 hover:shadow-lg touch-none
          ${isHovered ? "scale-125 shadow-lg" : "shadow-md"}
...
export default function GradientGenny() {
