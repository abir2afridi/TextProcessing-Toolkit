...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "All-in-one text processing toolkit: convert, clean, format, analyze, encode and generate text — entirely in the browser." },
      { property: "og:title", content: "tpt:// Text Processing Toolkit" },
    ...
    links: [
      ...
      { rel: "icon", type: "image/png", href: "https://img.icons8.com/?size=100&id=OfjTGv1SlHbW&format=png" },
    ],
  }),
...
function RootComponent() {
  ...
  useEffect(() => {
    ...
    try { localStorage.setItem("tpt-theme", theme); } catch {}
  }, [theme]);
        ...
        <div className="flex min-h-screen w-full bg-background">
          ...
          <div className="flex min-h-screen flex-1 flex-col">
            <header className="sticky top-0 z-30 flex h-12 items-center gap-2 border-b border-border bg-background/80 px-3 backdrop-blur">
              ...
              <Button variant="ghost" size="sm" onClick={toggleTheme} className="h-8 w-8 rounded-sm p-0" title={`Switch to ${theme === "dark" ? "light" : "dark"} mode`}>
